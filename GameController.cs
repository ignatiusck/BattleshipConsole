class GameController
{
    ...
    private Dictionary<int, Dictionary<string, IShip>>? _listShipsPlayer = new(); //full data relation player and all ships
    ...
    //create ship packet
    public Dictionary<string, IShip> CreateShipPack()
    {
    ...
    public void SetPlayerList(List<Player> _listPlayer)
    {
        foreach (Player player in _listPlayer)
        {
            ...
            _listShipsPlayer.Add(player.Id, ships);
        }
    }
    ...
    public bool AddToMap(string key, Coordinate coor, string rotate)
    {
        ...
        if (rotate == "V")
        {
            ...
            listShip[key].ShipCoordinates = listCoor;
    ...
    public Data HitEnemy(int x, int y)
    {
        ...
        if (shipCoor[x - 1, y - 1] != "_" && shipCoor[x - 1, y - 1] != "H" && shipCoor[x - 1, y - 1] != "*")
        {
            Coordinate c = new();
            c.SetValue(x - 1, y - 1);
            listShip[shipCoor[x - 1, y - 1]].ShipCoordinates.RemoveAll(coor => coor.GetValueX() == x - 1 && coor.GetValueY() == y - 1);

            if (listShip[shipCoor[x - 1, y - 1]].ShipCoordinates.Count() == 0)
            {
                data.Message = $" Opponent {listShip[shipCoor[x - 1, y - 1]].ShipName} ship Destroyed";
            }
